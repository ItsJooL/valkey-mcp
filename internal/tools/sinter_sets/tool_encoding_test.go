...
func TestSinterSetsTool_BinaryMember_IsBase64InJSON(t *testing.T) {
	mockClient := client.NewMockClient()
	tool := NewTool(mockClient)
	ctx := context.Background()

	// Both sets contain the binary member — intersection should include it
	mockClient.AddSet(ctx, "set1", []string{string(javaBinaryFixture), "text_member"})
	mockClient.AddSet(ctx, "set2", []string{string(javaBinaryFixture), "other_member"})
