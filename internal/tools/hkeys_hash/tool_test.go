...
func TestHkeysHash_Execute_Success(t *testing.T) {
	...
	assert.Contains(t, output.Result, "field1")
	assert.Contains(t, output.Result, "field2")
	assert.Contains(t, output.Result, "field3")
}

func TestHkeysHash_Execute_KeyNotFound(t *testing.T) {
	mockClient := client.NewMockClient()
	tool := NewTool(mockClient)
	ctx := context.Background()
...
func TestHkeysHash_Execute_EmptyKey(t *testing.T) {
	mockClient := client.NewMockClient()
	tool := NewTool(mockClient)
	ctx := context.Background()
