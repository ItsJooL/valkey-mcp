...
func TestLPopListTool_BinaryElement_IsBase64InJSON(t *testing.T) {
	...
	inputJSON, _ := json.Marshal(map[string]interface{}{"key": "mylist", "count": int64(1)})
	...
	_, isBytes := output.Elements[0].([]byte)
	...
	elements, ok := jsonResult["elements"].([]interface{})
	require.True(t, ok)
	require.Len(t, elements, 1)

	encodedStr, ok := elements[0].(string)
